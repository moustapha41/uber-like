-- ============================================
-- CONFIGURATION TARIFS PAR DÉFAUT
-- ============================================
-- Script pour insérer la configuration de tarification par défaut
-- À exécuter après la création des tables rides
...
-- Vérification
SELECT 'Configuration tarifs créée avec succès!' as status;
SELECT * FROM pricing_config WHERE service_type = 'ride' AND is_active = true;
SELECT * FROM pricing_time_slots WHERE pricing_config_id IN (
    SELECT id FROM pricing_config WHERE service_type = 'ride' AND is_active = true
);
