...
/// Service d'authentification branché sur l'API BikeRide Pro.
class AuthService {
  ...
  static const _keyToken = 'bikeride_token';
  static const _keyUser = 'bikeride_user';

  /// Token JWT actuel (null si non connecté).
  String? get token => _api.token ?? _storage.token;

  /// Client API configuré avec le token actuel (pour rides, deliveries, etc.).
  ApiClient get apiClient => _api;

  /// Utilisateur stocké (après login/register).
  Map<String, dynamic>? get user => _storage.user;

  /// Charge le token et l'utilisateur depuis le stockage local.
  ...
  /// Inscription (role = client pour l'app client).
  /// Retourne la réponse data (user + token).
  ...
  /// Connexion. Retourne data (user + token).
